...
export class GoldService {
  ...
  private priceCache: Map<string, GoldPricePoint[]> = new Map();
  private cacheExpiry: Map<string, number> = new Map();
  private readonly CACHE_DURATION = 5 * 60 * 1000; // 5 minutes

  getGoldPrices(from: string, to: string): Observable<GoldPricePoint[]> {
    ...
    return this.getRealGoldPrices(from, to).pipe(
      catchError(error => {
        console.warn('⚠️ Real API failed, using mock data:', error.message);
        return this.getDeterministicMockPrices(from, to);
      })
    );
  }

  private getRealGoldPrices(from: string, to: string): Observable<GoldPricePoint[]> {
    const cacheKey = `${from}-${to}`;
    ...
    console.log('🌐 Fetching real gold data from Metals-API:', { from, to });
    ...
    return this.http.get<MetalsApiHistoricalResponse>(url, { params }).pipe(
      map(response => {
        ...
        Object.entries(response.rates).forEach(([date, rates]) => {
          if (rates.XAU) {
            // Convert from USD per ounce to AUD per kg
            const pricePerOz = rates.XAU;
            const pricePerKg = pricePerOz * 32.1507; // Convert oz to kg
            const priceInAUD = pricePerKg * 1.5; // Approximate USD to AUD conversion
        ...
        console.log('✅ Real gold data received:', prices.length, 'points');
  ...
  private generateHistoricalData(from: string, to: string, currentPrice: number): GoldPricePoint[] {
    ...
    if (prices.length > 0) {
      ...
      console.log('📊 Gold Price Data Analysis:');
  ...
  private getDeterministicMockPrices(from: string, to: string): Observable<GoldPricePoint[]> {
    const cacheKey = `${from}-${to}`;
    
    // Generate deterministic mock data (same prices every time)
    const prices = this.generateHistoricalData(from, to, 204000); // Base price $204,000/kg
